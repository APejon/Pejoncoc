...
void	redir_add(t_parse *parse, t_shell shell, int counter, int redirects)
{
	...
	if (parse->pipe[counter][op] == '<' && parse->pipe[counter][op + 1] == '<')
	{
		...
		shell->command[counter]->redir[redirects]->file[redirects] = ft_strtrim(ft_substr(parse->pipe[counter], 0, op), ' ');
		free(parse->pipe[counter]);
		parse->pipe[counter] = ft_strtrim(ft_substr(string, op + 2, ft_strlen(string)), ' ');
		free(string);
	}
	if (parse->pipe[counter][op] == '<')
	{
		shell->command[counter]->redir[redirects]->direct = numbers;
		string = ft_strdup(parse->pipe[counter]);
		shell->command[counter]->redir[redirects]->file[redirects] = ft_strtrim(ft_substr(parse->pipe[counter], 0, op), ' ');
	...
	if (parse->pipe[counter][op] == '>' && parse->pipe[counter][op + 1] == '>')
	{
		shell->command[counter]->redir[redirects]->direct = numbers;
		string = ft_strdup(parse->pipe[counter]);
		shell->command[counter]->redir[redirects]->file[redirects] = ft_strtrim(ft_substr(parse->pipe[counter], op + 2, str_len(string)), ' ');
		free(parse->pipe[counter]);
		parse->pipe[counter] = ft_strtrim(ft_substr(string, 0, op), ' ');
		free(string);
	}
	if (parse->pipe[counter][op] == '>')
	{
		shell->command[counter]->redir[redirects]->direct = numbers;
		string = ft_strdup(parse->pipe[counter]);
		shell->command[counter]->redir[redirects]->file[redirects] = ft_strtrim(ft_substr(parse->pipe[counter], op + 1, str_len(string)), ' ');
		free(parse->pipe[counter]);
		parse->pipe[counter] = ft_strtrim(ft_substr(string, 0, op), ' ');
		free(string);
	}
